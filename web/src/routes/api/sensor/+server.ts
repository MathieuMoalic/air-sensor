...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const series = new Map<string, { id: string; name: string; unit: string; points: { ts: number; value: number; failed: number }[] }>();
  for (const s of sensors) {
    series.set(s.id, { id: s.id, name: s.name, unit: s.unit, points: [] });
  }

  for (const r of rows as ReadingRow[]) {
    let s = series.get(r.sensor_id);
    if (!s) {
      s = { id: r.sensor_id, name: r.name, unit: r.unit, points: [] };
      series.set(r.sensor_id, s);
    }
